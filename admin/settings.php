...
if ( isset( $_POST['s2_admin'] ) ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		...
		if ( ! empty( $this->subscribe2_options['last_s2cron'] ) || ( 'yes' === $this->subscribe2_options['stickies'] && ! empty( $stickies ) ) ) {
			...
			echo '<div id="message" class="updated fade"><p><strong>' . esc_html__( 'The Digest Notification email contained no post information. No email was sent', 'subscribe2-for-cp' ) . '</strong></p></div>';
		}
...
$template_link = '<a href="' . admin_url( 'admin.php?page=s2_settings&tab=templates' ) . '">' . __( 'Modify your template', 'subscribe2-for-cp' ) . '</a>';
if ( false !== $disallowed ) {
	$disallowed_keywords = __( 'Your chosen email type (per-post or digest) does not support the following keywords:', 'subscribe2-for-cp' );
...
// show our form
echo '<div class="wrap">';
echo '<h1>' . esc_html__( 'Settings', 'subscribe2-for-cp' ) . '</h1>' . "\r\n";
$s2tabs = array(
	'email'      => __( 'Email Settings', 'subscribe2-for-cp' ),
	'templates'  => __( 'Templates', 'subscribe2-for-cp' ),
	'registered' => __( 'Registered Users', 'subscribe2-for-cp' ),
	'appearance' => __( 'Appearance', 'subscribe2-for-cp' ),
	'misc'       => __( 'Miscellaneous', 'subscribe2-for-cp' ),
);
...
switch ( $current_tab ) {
	...
	case 'templates':
		...
		echo '<dl>';
		echo '<dt><b><em style="color: red">' . esc_html__( 'IF THE FOLLOWING KEYWORDS ARE ALSO IN YOUR POST THEY WILL BE SUBSTITUTED', 'subscribe2-for-cp' ) . '</em></b></dt><dd></dd>' . "\r\n";
		echo '<dt><b>{BLOGNAME}</b></dt><dd>' . esc_html( get_option( 'blogname' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{BLOGLINK}</b></dt><dd>' . esc_html( get_option( 'home' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TITLE}</b></dt><dd>' . wp_kses_post( __( "the post's title<br>(<i>for per-post emails only</i>)", 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TITLETEXT}</b></dt><dd>' . wp_kses_post( __( "the post's unformatted title <br>(<i>for per-post emails only</i>)", 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{POST}</b></dt><dd>' . wp_kses_post( __( "the excerpt or the entire post<br>(<i>based on the subscriber's preferences</i>)", 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{POSTTIME}</b></dt><dd>' . wp_kses_post( __( 'the excerpt of the post and the time it was posted<br>(<i>for digest emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TABLE}</b></dt><dd>' . wp_kses_post( __( 'a list of post titles<br>(<i>for digest emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TABLELINKS}</b></dt><dd>' . wp_kses_post( __( 'a list of post titles followed by links to the articles<br>(<i>for digest emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{REFERENCELINKS}</b></dt><dd>' . wp_kses_post( __( 'a reference style list of links at the end of the email with corresponding numbers in the content<br>(<i>for the full content plain text per-post email only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{PERMALINK}</b></dt><dd>' . wp_kses_post( __( "the post's permalink<br>(<i>for per-post emails only</i>)", 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TINYLINK}</b></dt><dd>' . esc_html__( "the post's permalink after conversion by TinyURL", 'subscribe2-for-cp' ) . '</dd>' . "\r\n";
		echo '<dt><b>{PERMAURL}</b></dt><dd>' . wp_kses_post( __( "the post's unformatted permalink<br>(<i>for per-post emails only</i>)", 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{DATE}</b></dt><dd>' . wp_kses_post( __( 'the date the post was made<br>(<i>for per-post emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{TIME}</b></dt><dd>' . wp_kses_post( __( 'the time the post was made<br>(<i>for per-post emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
		echo '<dt><b>{MYNAME}</b></dt><dd>' . esc_html__( "the admin or post author's name", 'subscribe2-for-cp' ) . '</dd>' . "\r\n";
		...
		echo '<dt><b>{CATS}</b></dt><dd>' . esc_html__( "the post's assigned categories", 'subscribe2-for-cp' ) . '</dd>' . "\r\n";
		echo '<dt><b>{TAGS}</b></dt><dd>' . esc_html__( "the post's assigned Tags", 'subscribe2-for-cp' ) . '</dd>' . "\r\n";
		echo '<dt><b>{COUNT}</b></dt><dd>' . wp_kses_post( __( 'the number of posts included in the digest email<br>(<i>for digest emails only</i>)', 'subscribe2-for-cp' ) ) . '</dd>' . "\r\n";
	...
	case 'registered':
		...
		echo '<h3>' . esc_html__( 'Compulsory Categories', 'subscribe2-for-cp' ) . '</h3>' . "\r\n";
		echo '<p>' . "\r\n";
		...
		echo '<h3>' . esc_html__( 'Excluded Categories', 'subscribe2-for-cp' ) . '</h3>' . "\r\n";
		echo '<p>';
		...
		if ( false !== $formats ) {
			...
			echo '<h3>' . esc_html__( 'Excluded Formats', 'subscribe2-for-cp' ) . '</h3>' . "\r\n";
			echo '<p>';
	...
	case 'misc':
		...
		echo '<a href="https://subscribe2.wordpress.com/latest-news/">' . esc_html__( 'Plugin Latest News', 'subscribe2-for-cp' ) . '</a><br>';
		echo '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&amp;hosted_button_id=2387904">' . esc_html__( 'Make a donation via PayPal', 'subscribe2-for-cp' ) . '</a>';
		echo '</div>' . "\r\n";
		break;

}
// submit
echo '<p class="submit" style="text-align: center"><input type="submit" class="button-primary" name="submit" value="' . esc_attr( __( 'Submit', 'subscribe2-for-cp' ) ) . '" /></p>';

if ( 'misc' === $current_tab ) {
	// reset
	echo '<h3>' . esc_html__( 'Reset to Default Settings', 'subscribe2-for-cp' ) . '</h3>' . "\r\n";
	echo '<p>' . esc_html__( 'Use this to reset all options to their defaults. This <strong><em>will not</em></strong> modify your list of subscribers.', 'subscribe2-for-cp' ) . '</p>' . "\r\n";
