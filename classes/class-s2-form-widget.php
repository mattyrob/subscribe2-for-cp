...
class S2_Form_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 's2_form_widget', esc_html__( 'Subscribe2 Widget', 'subscribe2-for-cp' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		$title             = empty( $instance['title'] ) ? __( 'Subscribe2', 'subscribe2-for-cp' ) : $instance['title'];
		...
		if ( 'subscribe' === $hidebutton || 'unsubscribe' === $hidebutton ) {
			...
			$hide = ' link="' . __( '(Un)Subscribe to Posts', 'subscribe2-for-cp' ) . '"';
		}
		$postid = '';
		if ( ! empty( $postto ) ) {
			$postid = ' id="' . $postto . '"';
	...
	public function form( $instance ) {
		...
		echo '>' . esc_html__( 'Use Home Page', 'subscribe2-for-cp' ) . '</option>' . "\r\n";
